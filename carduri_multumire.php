...
$cerereSQL = 'SELECT * FROM `subcategorii` WHERE `nume`="Carduri de multumire"';
...
if (isset($_GET['page']) && ($_GET['page'] == "prod")) {
	$cerereSQL3 = 'SELECT * FROM `subcategorii` WHERE `nume`="Carduri de multumire" ';
	...
	while ($rand = mysqli_fetch_assoc($rezultat)) {
		$poza = 'card_multumire/' . $rand['imagine'];
		...
		echo '
			<form action="carduri_multumire.php?page=pers" method="post">
	...
	if ((!isset($_POST['nume'])) || (!isset($_POST['pret']))) {
		...
		if (isset($_SESSION['PrenMire'])) $el = $_SESSION['PrenMire'];
		else $el = '(EL)';
		if (isset($_SESSION['PrenMireasa'])) $ea = $_SESSION['PrenMireasa'];
		else $ea = '(EA)';
		echo '
			<form action="carduri_multumire.php?page=factura" method="post">
				<p align="center"><font size="5"><b>Personalizare carduri de multumire</b></font></p>
				<p align="center"><img src="images/a.gif" alt=""></p>
				<table width="500" border="0" align="center" cellpading="2" cellspacing="3">
					...
					<tr>
						<td colspan="2" align="left">
							<font size="4"><b><i>Text</i></b></font>
						</td>
					</tr>
						<tr>
							<td align="right">&nbsp;</td>
							<td align="left">
								<textarea name="CMText" rows="6" cols="50" wrap="physical">Va multumim ca ati onorat
								cu prezenta Dvs. cel mai fericit eveniment
								din viata noastra.
								' . $el . ' si ' . $ea . '</textarea>
							</td>
						</tr>
	...
	if ((!isset($_POST['Submit']))) {
		...
		echo '
			<form action="carduri_multumire.php?page=finish" method="post" name="FormularText">
				...
				<table width="500" border="0" align="center" cellpading="2" cellspacing="1" style="font-size:9px;">
					...
					<tr>
					  <td align="left" valign="top" bgcolor="#DCFFA8">Carduri Multumire / ' . $_SESSION['numep'] . '</td>
					  <td align="center" valign="top" bgcolor="#DCFFA8">' . $_SESSION['CMText'] . '</td>
	...
	if (($_POST['Nume'] == '') || ($_POST['Adresa'] == '') || ($_POST['Oras'] == '') || ($_POST['cod_postal'] == '') || ($_POST['CNP'] == '') || ($_POST['Mail'] == '') || ($_POST['Tel'] == '') || (!is_numeric($_POST['CNP'])) || (!is_numeric($_POST['Tel'])) || (!is_numeric($_POST['cod_postal']))) {
		...
		$subiect = 'Factura Proforma (' . $_SESSION['cantitate'] . ' carduri de multumire ' . $_SESSION['numep'] . ')';
		$mesaj =
			'
		...
		$mesaj2 =
			...
			<table width="693" border="0" cellspacing="2" cellpadding="2" style="font-size:10px;">
			  ...
			  <tr>
				<td valign="top" bgcolor="#DCFFA8">Carduri Multumire : ' . $_SESSION['numep'] . '</td>
				<td align="center" valign="top" bgcolor="#DCFFA8">' . $_SESSION['CMText'] . '</td>
				<td align="right" valign="top" bgcolor="#DCFFA8">' . $_SESSION['cantitate'] . '</td>
				<td align="right" valign="top" bgcolor="#DCFFA8">' . $_SESSION['pretp'] . '</td>
				<td align="right" valign="top" bgcolor="#DCFFA8">' . $_SESSION['pret'] . '</td>
	...
	echo '<p align="center"><font size="5"><b>Carduri de multumire</b></font></p>
	<p align="center"><img src="images/a.gif" alt=""></p>';

	$intrari_totale2 = mysqli_num_rows(mysqli_query($conexiune, 'SELECT `id` FROM `articole` WHERE `pag`="Carduri de multumire"'));
	if ($intrari_totale2 == 0) {
		echo '';
	} else {
		$cerereSQL = 'SELECT * FROM `articole` WHERE `pag`="Carduri de multumire"';
		$rezultat = mysqli_query($conexiune, $cerereSQL);
		while ($rand = mysqli_fetch_assoc($rezultat)) {
			echo '
			...
			if ($pagini_totale > 0) {
				...
				while ($rand = mysqli_fetch_assoc($rezultat)) {
					...
					'<td align="center" width="185">
							<a href="carduri_multumire.php?page=prod&id=' . $rand['id'] . '"><img src="card_multumire/thumb/' . $rand['imagine'] . '" width="100" alt=""></a><br>
				...
				if ($pagini_totale == 1) echo '<div align=left> </div>';
					...
					for ($pagini = 1; $pagini <= $pagini_totale; $pagini++) {
						...
						else echo '<a href="carduri_multumire.php?p=' . $pagini . '">' . $pagini . '</a>&nbsp;';
					}
					echo '</div>';
					...
					if ($pagina > 1) {
						$inapoi = ($pagina - 1);
						echo '<a href="carduri_multumire.php?p=' . $inapoi . '"><img src="images/anterioara.gif" width="30" height="30"></a>';
					}
					echo '</td>
							<td align="right">';
					if ($pagina < $pagini_totale) {
						$inainte = ($pagina + 1);
						echo '<a href="carduri_multumire.php?p=' . $inainte . '"><img src="images/urmatoare.gif" width="30" height="30"></a>';
					}
					echo '</td>
						  </tr></table>';
				}
