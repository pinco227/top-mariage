...
if ($intrari_totale2 == 0) {
	...
	$cerereSQL = 'SELECT * FROM `articole` WHERE `pag`="Artificii"';
	$rezultat = mysqli_query($conexiune, $cerereSQL);
	while ($rand = mysqli_fetch_assoc($rezultat)) {
		echo '
...
if ($intrari_totale == 0) {
	...
	'
			...
			';
	$nr = 0;
	$cerereSQL = 'SELECT * FROM `imagini` WHERE `pentru`="artificii"';
	$rezultat = mysqli_query($conexiune, $cerereSQL);
	while ($rand = mysqli_fetch_assoc($rezultat)) {
		$nr++;
		echo
		'<td align="center" width="185">
							<a href="artificii/' . $rand['path'] . '" rel="lightbox[Artificii]" title="' . $rand['nume'] . '"><img src="artificii/thumb/' . $rand['path'] . '" height="100" alt=""></a>
						</td>';
