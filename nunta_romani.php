...
$cerereSQL = 'SELECT * FROM `subcategorii` WHERE `nume`="Nunta la romani"';
...
$intrari_totale2 = mysqli_num_rows(mysqli_query($conexiune, 'SELECT `id` FROM `articole` WHERE `pag`="Nunta la romani"'));
if ($intrari_totale2 == 0) {
	echo '';
} else {
	$cerereSQL = 'SELECT * FROM `articole` WHERE `pag`="Nunta la romani"';
	$rezultat = mysqli_query($conexiune, $cerereSQL);
	while ($rand = mysqli_fetch_assoc($rezultat)) {
		echo '
...
$intrari_totale = mysqli_num_rows(mysqli_query($conexiune, 'SELECT `nume` FROM `imagini` WHERE `pentru`="nunta_romani"'));
if ($intrari_totale == 0) {
	echo '';
} else {
	echo
	'
			...
			';
	$nr = 0;
	$cerereSQL = 'SELECT * FROM `imagini` WHERE `pentru`="nunta_romani"';
	$rezultat = mysqli_query($conexiune, $cerereSQL);
	while ($rand = mysqli_fetch_assoc($rezultat)) {
		$nr++;
		echo
		'<td align="center" width="185">
							<a href="nunta_romani/' . $rand['path'] . '" rel="lightbox[Nunta la romani]" title="' . $rand['nume'] . '"><img src="nunta_romani/thumb/' . $rand['path'] . '" height="100" alt=""></a>
						</td>';
