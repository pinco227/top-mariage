...
$cerereSQL = 'SELECT * FROM `subcategorii` WHERE `nume`="Sonorizari"';
...
$intrari_totale2 = mysqli_num_rows(mysqli_query($conexiune, 'SELECT `id` FROM `articole` WHERE `pag`="Sonorizari"'));
if ($intrari_totale2 == 0) {
	echo '';
} else {
	$cerereSQL = 'SELECT * FROM `articole` WHERE `pag`="Sonorizari"';
	$rezultat = mysqli_query($conexiune, $cerereSQL);
	while ($rand = mysqli_fetch_assoc($rezultat)) {
		echo '
...
$intrari_totale = mysqli_num_rows(mysqli_query($conexiune, 'SELECT `nume` FROM `imagini` WHERE `pentru`="sonorizari"'));
if ($intrari_totale == 0) {
	echo '';
} else {
	echo
	'
			...
			';
	$nr = 0;
	$cerereSQL = 'SELECT * FROM `imagini` WHERE `pentru`="sonorizari"';
	$rezultat = mysqli_query($conexiune, $cerereSQL);
	while ($rand = mysqli_fetch_assoc($rezultat)) {
		$nr++;
		echo
		'<td align="center" width="185">
							<a href="sonorizari/' . $rand['path'] . '" rel="lightbox[Sonorizari]" title="' . $rand['nume'] . '"><img src="sonorizari/thumb/' . $rand['path'] . '" height="100" alt=""></a>
						</td>';
