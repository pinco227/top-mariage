...
$cerereSQL = 'SELECT * FROM `subcategorii` WHERE `nume`="Plicuri Bani"';
...
if (isset($_GET['page']) && ($_GET['page'] == "prod")) {
	$cerereSQL3 = 'SELECT * FROM `subcategorii` WHERE `nume`="Plicuri Bani" ';
	...
	while ($rand = mysqli_fetch_assoc($rezultat)) {
		...
		echo '
			<form action="plicuri_bani.php?page=pers" method="post">
	...
	if ((!isset($_POST['nume'])) || (!isset($_POST['pret']))) {
		...
		echo '
			<form action="plicuri_bani.php?page=factura" method="post">
				<p align="center"><font size="5"><b>Personalizare plicuri pentru bani</b></font></p>
				<p align="center"><img src="images/a.gif" alt=""></p>
				<table width="500" border="0" align="center" cellpading="2" cellspacing="3">
					...
					<tr>
						...
						<tr>
							<td align="right">&nbsp;</td>
							<td align="left">
								<textarea name="CPText" rows="6" cols="50" wrap="physical">' . $el . ' si ' . $ea . '  va doresc o petrecere de neuitat
								si va multumesc ca sunteti alaturi de ei
								 Distractie placuta!</textarea>
							</td>
						</tr>
					<tr>
						<td colspan="2" align="left">
		...
		echo '
			<form action="plicuri_bani.php?page=finish" method="post" name="FormularText">
				...
				<table width="500" border="0" align="center" cellpading="2" cellspacing="1" style="font-size:9px;">
					...
					<tr>
					  <td align="left" valign="top" bgcolor="#DCFFA8">Plicuri bani / ' . $_SESSION['numep'] . '</td>
					  <td align="center" valign="top" bgcolor="#DCFFA8">' . $_SESSION['CPText'] . '</td>
	...
	if (($_POST['Nume'] == '') || ($_POST['Adresa'] == '') || ($_POST['Oras'] == '') || ($_POST['cod_postal'] == '') || ($_POST['CNP'] == '') || ($_POST['Mail'] == '') || ($_POST['Tel'] == '') || (!is_numeric($_POST['CNP'])) || (!is_numeric($_POST['Tel'])) || (!is_numeric($_POST['cod_postal']))) {
		...
		$subiect = 'Factura Proforma (' . $_SESSION['cantitate'] . ' plicuri pentru bani ' . $_SESSION['numep'] . ')';
		$mesaj =
			'
		...
		$mesaj2 =
			...
			<table width="693" border="0" cellspacing="2" cellpadding="2" style="font-size:10px;">
			  ...
			  <tr>
				<td valign="top" bgcolor="#DCFFA8">Plicuri Bani : ' . $_SESSION['numep'] . '</td>
				<td align="center" valign="top" bgcolor="#DCFFA8">' . $_SESSION['CPText'] . '</td>
				<td align="right" valign="top" bgcolor="#DCFFA8">' . $_SESSION['cantitate'] . '</td>
				<td align="right" valign="top" bgcolor="#DCFFA8">' . $_SESSION['pretp'] . '</td>
				<td align="right" valign="top" bgcolor="#DCFFA8">' . $_SESSION['pret'] . '</td>
	...
	echo '<p align="center"><font size="5"><b>Plicuri pentru bani</b></font></p>
	<p align="center"><img src="images/a.gif" alt=""></p>';

	$intrari_totale2 = mysqli_num_rows(mysqli_query($conexiune, 'SELECT `id` FROM `articole` WHERE `pag`="Plicuri Bani"'));
	if ($intrari_totale2 == 0) {
		echo '';
	} else {
		$cerereSQL = 'SELECT * FROM `articole` WHERE `pag`="Plicuri Bani"';
		$rezultat = mysqli_query($conexiune, $cerereSQL);
		while ($rand = mysqli_fetch_assoc($rezultat)) {
			echo '
			...
			if ($pagini_totale > 0) {
				...
				while ($rand = mysqli_fetch_assoc($rezultat)) {
					...
					'<td align="center" width="185">
							<a href="plicuri_bani.php?page=prod&id=' . $rand['id'] . '"><img src="plic_bani/thumb/' . $rand['imagine'] . '" width="100" alt=""></a><br>
				...
				if ($pagini_totale == 1) echo '<div align=left> </div>';
					...
					for ($pagini = 1; $pagini <= $pagini_totale; $pagini++) {
						...
						else echo '<a href="plicuri_bani.php?p=' . $pagini . '">' . $pagini . '</a>&nbsp;';
					}
					echo '</div>';
					...
					if ($pagina > 1) {
						$inapoi = ($pagina - 1);
						echo '<a href="plicuri_bani.php?p=' . $inapoi . '"><img src="images/anterioara.gif" width="30" height="30"></a>';
					}
					echo '</td>
							<td align="right">';
					if ($pagina < $pagini_totale) {
						$inainte = ($pagina + 1);
						echo '<a href="plicuri_bani.php?p=' . $inainte . '"><img src="images/urmatoare.gif" width="30" height="30"></a>';
					}
					echo '</td>
						  </tr></table>';
				}
